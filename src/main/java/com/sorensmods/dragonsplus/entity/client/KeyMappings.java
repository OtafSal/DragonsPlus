...
{
    private static final String KEY_CATEGORY = "key.category." + DragonsPlus.MOD_ID;

    public static final KeyMapping FLIGHT_DESCENT_KEY = keymap("flight_descent", GLFW.GLFW_KEY_Z);
    public  static final KeyMapping FLIGHT_ASCEND_KEY = keymap("flight ascend", GLFW.GLFW_KEY_SPACE);
    ...
    {
        return new KeyMapping(String.format("key.%s.%s", DragonsPlus.MOD_ID, name), defaultMapping, KEY_CATEGORY);
    }

    public static void registerKeybinds(Consumer<KeyMapping> registrar)
    {
        registrar.accept(FLIGHT_DESCENT_KEY);
        registrar.accept(FLIGHT_ASCEND_KEY);
